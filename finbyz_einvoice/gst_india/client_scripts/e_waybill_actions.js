function setup_e_waybill_actions(doctype) {
    ...
    frappe.ui.form.on(doctype, {
        ...
        setup(frm) {
            if (!finbyz_einvoice.is_api_enabled()) return;
        ...
        refresh(frm) {
            ...
            if (!finbyz_einvoice.is_api_enabled() || !is_e_waybill_generated_using_api(frm)) {
                return;
            }
        ...
        async on_submit(frm) {
            if (
                ...
                frm.doc.is_return ||
                frm.doc.is_debit_note ||
                !finbyz_einvoice.is_api_enabled() ||
            ...
            await frappe.xcall(
                "finbyz_einvoice.gst_india.utils.e_waybill.generate_e_waybill",
                { doctype: frm.doctype, docname: frm.doc.name }
            );
        },
        before_cancel(frm) {
            // if IRN is present, e-Waybill gets cancelled in e-Invoice action
            if (!finbyz_einvoice.is_api_enabled() || frm.doc.irn || !frm.doc.ewaybill) return;
...
function fetch_e_waybill_data(frm, args, callback) {
    ...
    frappe.call({
        method: "finbyz_einvoice.gst_india.utils.e_waybill.fetch_e_waybill_data",
...
function show_generate_e_waybill_dialog(frm) {
    const generate_action = values => {
        frappe.call({
            method: "finbyz_einvoice.gst_india.utils.e_waybill.generate_e_waybill",
            args: {
                doctype: frm.doctype,
    ...
    const json_action = async values => {
        const ewb_data = await frappe.xcall(
            "finbyz_einvoice.gst_india.utils.e_waybill.generate_e_waybill_json",
            {
                doctype: frm.doctype,
    ...
    const api_enabled = finbyz_einvoice.is_api_enabled();
...
function show_cancel_e_waybill_dialog(frm, callback) {
    const d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frappe.call({
                method: "finbyz_einvoice.gst_india.utils.e_waybill.cancel_e_waybill",
                args: {
                    doctype: frm.doctype,
                    docname: frm.doc.name,
                    values,
...
function show_update_vehicle_info_dialog(frm) {
    const d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frappe.call({
                method: "finbyz_einvoice.gst_india.utils.e_waybill.update_vehicle_info",
                args: {
                    doctype: frm.doctype,
                    docname: frm.doc.name,
                    values,
...
function show_update_transporter_dialog(frm) {
    const d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frappe.call({
                method: "finbyz_einvoice.gst_india.utils.e_waybill.update_transporter",
                args: {
                    doctype: frm.doctype,
                    docname: frm.doc.name,
                    values,
...
function get_primary_action_label_for_generation(doc) {
    const label = finbyz_einvoice.is_api_enabled() ? __("Generate") : __("Download JSON");
