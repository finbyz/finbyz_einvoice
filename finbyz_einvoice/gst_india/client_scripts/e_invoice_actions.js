frappe.ui.form.on("Sales Invoice", {
    refresh(frm) {
        ...
        if (
            ...
            frm.add_custom_button(
                ...
                () => {
                    frappe.call({
                        method: "finbyz_einvoice.gst_india.utils.e_invoice.generate_e_invoice",
    ...
    before_cancel(frm) {
        ...
        return new Promise(resolve => {
            ...
            if (!is_irn_cancellable(frm) || !finbyz_einvoice.is_e_invoice_enabled()) {
                const d = frappe.warn(
...
function show_cancel_e_invoice_dialog(frm, callback) {
    const d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frappe.call({
                method: "finbyz_einvoice.gst_india.utils.e_invoice.cancel_e_invoice",
                args: {
                    docname: frm.doc.name,
                    values: values,
                },
...
function is_e_invoice_applicable(frm) {
    return (
        finbyz_einvoice.is_e_invoice_enabled() &&
        frm.doc.docstatus == 1 &&
