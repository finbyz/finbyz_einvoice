    ...
    frappe.ui.form.on(doctype, {
        after_save(frm) {
            ...
            frappe.confirm(message, function () {
                frappe.call({
                    method: "finbyz_einvoice.gst_india.overrides.party.update_docs_with_previous_gstin",
                    args: {
                        gstin: gstin || "",
                        gst_category,
                        docs_with_previous_gstin,
...
function set_gstin_query(doctype) {
    frappe.ui.form.on(doctype, {
        async refresh(frm) {
            ...
            field.set_data(await finbyz_einvoice.get_gstin_options(frm.doc.name, doctype));
        },
    });
}
...
function set_gst_category(doctype) {
    frappe.ui.form.on(doctype, {
        gstin(frm) {
            frm.set_value(
                "gst_category",
                finbyz_einvoice.guess_gst_category(frm.doc.gstin, frm.doc.country)
            );
        },
    });
}
