...
from finbyz_einvoice.gst_india.utils.api import enqueue_integration_request
...
class BaseAPI:
    ...
    def handle_http_code(self, status_code, response_json):
        ...
        if status_code == 401 or (
            ...
            frappe.throw(
                _("Error establishing connection to GSP. Please contact {0}.").format(
                    ...
                    else _("Finbyz Einvoice API Support")
                ),
        ...
        if status_code == 429:
            frappe.throw(
                _("Your Finbyz Einvoice API credits have exhausted"),
                title=_("API Credits Exhausted"),
            )

        if status_code == 403:
            frappe.throw(
                _("Your Finbyz Einvoice API key is invalid"),
