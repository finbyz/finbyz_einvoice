...
frappe.query_reports["GSTR-1"] = {
	"filters": [
		...
		},
		{
			...
			"get_query": function () {
				const company = frappe.query_report.get_filter_value('company');
				return finbyz_einvoice.get_gstin_query(company);
			}
		},
		{
			"fieldname": "from_date",
	...
	onload: function (report) {
		report.page.add_inner_button(__("Download as JSON"), function () {
			frappe.call({
				method: 'finbyz_einvoice.gst_india.report.gstr_1.gstr_1.get_json',
				args: {
					data: report.data,
				...
				callback: function(r) {
					if (r.message) {
						const args = {
							cmd: 'finbyz_einvoice.gst_india.report.gstr_1.gstr_1.download_json_file',
							data: r.message.data,
