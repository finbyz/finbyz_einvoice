	...
	refresh : function(frm) {
		...
		if(!frm.is_new()) {
			...
			frm.add_custom_button(__('Download JSON'), function() {
				var w = window.open(
					frappe.urllib.get_full_url(
						"/api/method/finbyz_einvoice.gst_india.doctype.gstr_3b_report.gstr_3b_report.make_json?"
			...
			frm.add_custom_button(__('View Form'), function() {
				frappe.call({
					"method" : "finbyz_einvoice.gst_india.doctype.gstr_3b_report.gstr_3b_report.view_report",
					"args" : {
						name : frm.doc.name,
					},
					"callback" : function(r){
